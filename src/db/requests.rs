    ...
    A: Acquire<'a, Database = Sqlite>,
{
    ...
    sqlx::query(
        ...
        INSERT INTO messages (
            chat_id, message_id, reply_to_id, user_id, user_name, content, quote, timestamp,
            forward_from
        ) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    A: Acquire<'a, Database = Sqlite>,
{
    ...
    let thread_row = sqlx::query("SELECT summary FROM chats WHERE chat_id = ?")
        .bind(chat_id)
...
pub async fn archive_chat_history<'a, A>(
    conn: A,
    chat_id: i64,
    new_summary: &str,
    last_id: i64,
    ...
    A: Acquire<'a, Database = Sqlite>,
{
    ...
    sqlx::query("UPDATE chats SET summary = ? WHERE chat_id = ?")
        .bind(new_summary)
        .bind(chat_id)
        .execute(&mut *tx)
        .await?;

    let result = sqlx::query("DELETE FROM messages WHERE chat_id = ? AND message_id <= ?")
        .bind(chat_id)
        .bind(last_id)
        .execute(&mut *tx)
        .await?;
